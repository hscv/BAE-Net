...
def crop_image2(img, bbox, img_size=107, padding=16, flip=False, rotate_limit=0, blur_limit=0):
    ...
    imgArr = [img[:,:,k*3:k*3+3] for k in range(5)]
    pathArr = []
    # print ('--------------------')
    for img in imgArr:
        ...
        pathArr.append(patch)

    tt = np.zeros((pathArr[0].shape[0],pathArr[0].shape[1],1))
    # print ('tt.shape = ',tt.shape)
    patch = np.concatenate((pathArr[0], pathArr[1], pathArr[2], pathArr[3],pathArr[4],tt), axis=2)
